...
def handle_task(task, client_sock):
    start_time = time.time()
    time_limit = task["time_limit"]
    history = task.get("history", [])

    print(f"Worker 0 - Procesando tarea con tiempo límite de {time_limit} segundos.")

    completed, state = process_task(task, start_time, time_limit)
    elapsed_time = time.time() - start_time

    history.append({"worker": "Worker 0", "time": elapsed_time})

    if completed:
        print(f"Worker 0 - Ordenamiento completado en {elapsed_time:.4f} segundos.")

        # Guardar el vector ordenado en un archivo
        output_filename = "vector_ordenado_Worker_0.txt"
        save_vector_to_file(task["data"], output_filename)

        result = {
            "sorted_data": None,  # No enviamos el vector completo para evitar problemas
            "completed_by": "Worker 0",
            "history": history,
            "filename": output_filename
        }
        send_data(client_sock, result)
    else:
        print(f"Worker 0 - Tiempo límite excedido después de {elapsed_time:.4f} segundos. Delegando tarea a Worker 1.")
        task["state"] = state
        task["history"] = history
        partial_response = {
            "partial": True,
            "message": "Worker 0 ha delegado la tarea a Worker 1.",
            "time": elapsed_time
        }
        send_data(client_sock, partial_response)
        # Reenviar la tarea al siguiente worker
        response = forward_to_worker(task, WORKER_1_ADDRESS[0], WORKER_1_ADDRESS[1])
...
def handle_client(client_sock):
    try:
        ...
        handle_task(task, client_sock)
    except Exception as e:
        print(f"Error al manejar la tarea del cliente: {e}")
        send_data(client_sock, {"error": f"Error en Worker 0: {e}"})
    finally:
        client_sock.close()

def main():
    server_socket = socket.socket(socket.AF_INET, socket.SOCK_STREAM)
    server_socket.bind(WORKER_0_ADDRESS)
    server_socket.listen(5)
    print(f"Worker 0 escuchando en {WORKER_0_ADDRESS}...")
